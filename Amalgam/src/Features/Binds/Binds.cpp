...
static inline void LoopVars(int iBind, std::vector<BaseVar*>& vVars = G::Vars)
{
	...
	for (auto pBase : vVars)
	{
		...
		else Set(VA_LIST(std::vector<std::pair<std::string, MaterialColor_t>>), iBind)
...
void CBinds::RemoveBind(int iBind, bool bForce)
{
	if (!bForce)
	{
		for (auto& pBase : G::Vars)
		{
			if (HasType(bool, iBind)
				...
				|| HasType(VA_LIST(std::vector<std::pair<std::string, MaterialColor_t>>), iBind)
	...
	auto fRemoveBind = [&](int iIndex)
	{
		...
		for (auto& pBase : G::Vars)
		{
			...
			else Remove(VA_LIST(std::vector<std::pair<std::string, MaterialColor_t>>), iIndex)
...
void CBinds::Move(int i1, int i2)
{
	...
	for (auto& pBase : G::Vars)
	{
		...
		else Swap(VA_LIST(std::vector<std::pair<std::string, MaterialColor_t>>), i1, i2)
