...
public:
	...
	std::vector<int>& GetPlayerTags(uint32_t uAccountID) { auto it = m_mPlayerTags.find(uAccountID); return it != m_mPlayerTags.end() ? it->second : m_vDummy; }
	std::string* GetPlayerAlias(uint32_t uAccountID) { auto it = m_mPlayerAliases.find(uAccountID); return it != m_mPlayerAliases.end() ? &it->second : nullptr; }
};
