...
struct Group_t
{
	...
	// Bit position in Vars::ESP::ActiveGroups, assigned once per group and kept
	// stable across reordering/duplication/deletion so keybinds referencing this
	// group's bit don't silently start pointing at a different group.
	int m_iSlot = -1;
...
public:
	...
	int NextSlot(); // lowest bit position (0-31) not currently used by any group
