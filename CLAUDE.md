...
3. **Composite** (`DrawComposite()`, called from the `IEngineVGui_Paint` hook): builds the on-screen warp by sampling captured face textures through the inverse (`ScreenToRay`) reprojection — Panini for FOV ≤ 170°, Mercator for FOV ≥ 190°, linearly blended between. `DrawDebug()` blits the raw face thumbnails instead, for verification.
